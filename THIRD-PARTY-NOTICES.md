...
- **Files:** `packages/runtime/bin/node.dll`, `native/third_party/miniblink/wke.h`
...
- **SHA-256 (`node.dll`):**
  ...
  (also recorded in `packages/runtime/bin/node.dll.sha256`)
...
- **Files:** `packages/runtime/nsis/**` — `makensis.exe`, `Stubs/`, `Include/`,
  `Plugins/x86-unicode/`, `Bin/`, `Contrib/Graphics/`
- **Upstream:** https://nsis.sourceforge.io/
- **Version:** 3.10, Unicode
- **License:** see `packages/runtime/nsis/COPYING`, redistributed verbatim
