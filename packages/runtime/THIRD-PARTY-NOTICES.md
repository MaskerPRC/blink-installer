...
- **Files:** `./bin/node.dll`, the `wke.h` header used to build `blinkkit.dll`
...
- **SHA-256 (`node.dll`):**
  ...
  (also recorded in `./bin/node.dll.sha256`)
